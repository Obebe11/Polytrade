...
ВАЖНО — типы подписи (signature_type в .env):
  ...
  2 = GNOSIS_SAFE — аккаунт Polymarket через MetaMask/Rabby ← скорее всего ваш

FUNDER_ADDRESS = ваш proxy-адрес из polymarket.com/settings
"""
...
# Proxy-адрес из polymarket.com/settings (левый угол профиля)
FUNDER_ADDRESS   = os.getenv("FUNDER_ADDRESS")

# Тип подписи: 0=EOA, 1=POLY_PROXY, 2=GNOSIS_SAFE
...
def parse_token_ids(raw) -> list:
    """Безопасно парсит clobTokenIds — строка или список."""
    if not raw:
        return []
...
def calc_size(portfolio: float, price: float) -> float:
    if price <= 0:
        return 0.0
    return max(round((portfolio * RISK_PCT) / price, 2), 1.0)
...
class PolymarketBot:
    ...
    def __init__(self):
        ...
        # funder = proxy-адрес если задан, иначе адрес кошелька
    ...
    def run_once(self):
        ...
        for m in markets:
            ...
            log.info(f"✔ [{cat.upper()}] {question[:60]} | ask={best_ask:.4f} | days={days:.0f}")
...
if __name__ == "__main__":
    ...
    if not FUNDER_ADDRESS:
        log.warning("FUNDER_ADDRESS не задан — используется адрес кошелька. Если деньги на proxy — укажи FUNDER_ADDRESS в .env")
